...
import { BaseEntryWithTags } from "../types.ts";

export async function generateTagsForEntry(
  entry: { title: string; description: string; url: string },
): Promise<BaseEntryWithTags> {
  try {
    ...
    const parts = [
      {
        text:
          "Your job is to add tags to newsletter entries. Output only in JSON. Change nothing about the original input. Add as many tags as you deem necessary, but no more than 5. ",
      },
      {
        text:
